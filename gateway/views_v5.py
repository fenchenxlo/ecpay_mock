...
#     print("payment_notification: ",payment_notification)
    ...
    payment_payload = json.dumps(payment_payload, ensure_ascii=False, default=str)
    content = {"payment_payload": payment_payload}
    print("payment_payload: ", payment_payload)
#     content = {
#         "content_js_dict": payment_payload,  # 原始 dict，給模板顯示用
#         "content_js_json": json.dumps(payment_payload, ensure_ascii=False, default=str)  # JSON 字串，給 JS 用
#     }
...
def download_orderPayment_json(request):
    ...
    # 1. 從 URL 參數取得商店訂單編號 (例如: ?trade_no=202606110001)
    merchant_trade_no = request.GET.get("merchant_trade_no")

    if not merchant_trade_no:
        return JsonResponse({"error": "請提供 trade_no 參數"}, status=400)
