...
from .models import Order,PaymentRequest, PaymentNotification ,VirtualAccount          # 匯入 Order model，用來更新訂單狀態
...
def notify_bank(payment_request):
    ...
    payload = {
        ...
        "trade_amt": int(payment_request.trade_amt),

        "expire_date": payment_request.expire_date.isoformat()
    }

    payload["CheckMacValue"] = (
        generate_check_mac_value(payload)
    )
    ...
    try:
        ...
        response = httpx.post(
            ...
            "http://127.0.0.1:8002/create-account/",
