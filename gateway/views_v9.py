...
# Gateway呼叫Bank API
@csrf_exempt
def notify_bank(payment_request):
    """
    通知銀行建立虛擬帳號
    """
...
#             payment_data = json.loads(request.body.decode("utf-8"))
            ...
            res_data = response.POST.dict() if response.POST else response.body
                # 如果商城端用 JSON 傳送
    ...
    #            payment_data  = json.loads(request.body.decode("utf-8"))
            ...
            return HttpResponseBadRequest(f"JSON 格式錯誤: {res_data}")
        
#         res_status = response.text
#         res_status = json.loads(res_status.decode("utf-8"))
        
        if res_status["status"] == "failed":
            return JsonResponse({
            "status":"failed"
        })
        print(res_status)
...
# 
#         merchant_trade_date=timezone.now(),
# 
#         trade_amt=order.amount,
# 
#         trade_desc="ATM付款",
# 
#         item_name="商品",
...
#         bank_code="013",
# 
#         v_account=generate_virtual_account(),
# 
...
#     )
# 
#     notify_bank(payment_request)
# 
#     return render(request, "atm_info.html", {"payment": payment_request})
...
#     print("payment_data: " ,payment_data)
    ...
    with transaction.atomic():
        ...
        res_status = notify_bank(payment_request)
        print("res_status type: ", res_status)
