...
#             'expire_date': expire_date,
    ...
    if payment_request.bank_code == None and payment_request.v_account == None and payment_request.expire_date == None:
        # 產生 ATM 虛擬帳號
        payment_request.bank_code = bank_code = "013"  # 國泰世華
        payment_request.v_account = v_account = f"{bank_code}{random.randint(100000000000,999999999999)}"
        payment_request.expire_date = expire_date = (datetime.datetime.now() + datetime.timedelta(days=3)).strftime("%Y-%m-%d %H:%M:%S")
...
#     content = {'content_js': content_js}
    ...
    return render(request, "ecpay_atm_info.html", content)
...
def simulate_bank_paid(request):
    ...
    merchant_trade_no = context.get("merchant_trade_no") # 訂單編號
    ...
    order, created = Order.objects.get_or_create(
            order_number=merchant_trade_no,  # 查找條件
            defaults={
                'user_id': context["user_id"],
                'receiver_name': "王大明",
                'amount': context["trade_amt"],
                'status': "paid",
            }
        )
    print("3")
...
#         return HttpResponse("已付款")    
        ...
        PaymentNotification.objects.create(
            ...
            choose_payment=payment.choose_payment,
    ...
    print("PaymentNotification: ",PaymentNotification)
...
#     content = {
#         "content_js_dict": context,  # 原始 dict，給模板顯示用
#         "content_js_json": json.dumps(context, ensure_ascii=False, default=str)  # JSON 字串，給 JS 用
#     }
